  ...
  render() {
    return (
      <CartContext.Consumer>
        ...
        {(context) => (
          <>
            <h1 style={{opacity:backgroundColor}} className="categoryTitle"> Category {this.state.category}</h1>
            <div style={{opacity:backgroundColor}} className="grid_container">
              {this.state.products.map((item, index) => {
                return (
                  <div key={index} style={{backgroundColor:backgroundColor}} className="grid_items">
                    {item.inStock === true ? (
                      <>
                        <Link className="link" to={`/pdp?productID=${item.id}`}>
                          ...
                          <div className="card_content">
                            <h4 className="card_content_title">{item.name}</h4>
                            {item.prices.map((item, index) => {
                              return (
                                <div key={index}>
                                  {item.currency.label === context.currency && (
                                    <p key="index" className="card_content_price">                                                                                                             
